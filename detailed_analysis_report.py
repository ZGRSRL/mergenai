...
def create_detailed_analysis_report():
    """Detaylı doküman analizi raporu oluştur"""
    print("=" * 80)
    print("DETAYLI DOKUMAN ANALIZI RAPORU OLUSTURULUYOR")
    print("=" * 80)
    
    # PDF dosya adı
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    pdf_filename = f"FLETC_Artesia_Lodging_Detailed_Analysis_{timestamp}.pdf"
    ...
    try:
        ...
        story.append(Paragraph("Detayli Dokuman Analizi ve Teklif Raporu", styles['Heading2']))
...
def main():
    """Ana fonksiyon"""
    try:
        pdf_filename = create_detailed_analysis_report()
        
        if pdf_filename:
            print("Detayli rapor basariyla olusturuldu!")
