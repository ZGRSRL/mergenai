#!/usr/bin/env python3
"""
Streamlit Live AutoGen Demo - Otel Arama Agent ile
"""
...
def search_nearby_hotels(location, radius_km=50):
    """Fırsatın bulunduğu yerdeki otelleri ara"""
    
    # Demo otel verileri (gerçek uygulamada Google Places API veya benzeri kullanılır)
    demo_hotels = {
        "Washington DC": [
            {
    ...
    # Location'a göre otelleri döndür
    for key, hotels in demo_hotels.items():
        if key.lower() in location.lower() or location.lower() in key.lower():
            return hotels
    
    # Varsayılan olarak Washington DC otellerini döndür
    return demo_hotels["Washington DC"]
...
def create_executive_pdf_report(results, total_metrics, hotel_data=None):
    ...
    story.append(Paragraph("Üst Yönetim Raporu + Otel Analizi", title_style))
    ...
    story.append(Paragraph(
        ...
        f"compliance oranı %{total_metrics['compliance_rate']:.1f} seviyesindedir. Ayrıca her fırsat için "
        f"yakındaki otel seçenekleri analiz edildi.",
        normal_style
    ))
    ...
    metrics_data = [
        ...
        ['Analiz Edilen Otel', str(len(hotel_data) if hotel_data else 0), 'Yakındaki otel seçenekleri'],
    ...
    # Otel Analizi
    if hotel_data:
        story.append(Paragraph("OTEL ANALİZİ", heading_style))
        
        hotel_data_table = [
            ['Otel Adı', 'Adres', 'Puan', 'Fiyat Aralığı', 'Kapasite', 'Mesafe']
        ]
        
        for hotel in hotel_data[:5]:  # İlk 5 oteli göster
            hotel_data_table.append([
                hotel['name'][:30] + '...' if len(hotel['name']) > 30 else hotel['name'],
                hotel['address'][:25] + '...' if len(hotel['address']) > 25 else hotel['address'],
                str(hotel['rating']),
                hotel['price_range'],
                str(hotel['capacity']),
                hotel['distance']
        ...
        hotel_table = Table(hotel_data_table, colWidths=[1.8*inch, 1.8*inch, 0.8*inch, 1.2*inch, 0.8*inch, 0.8*inch])
    ...
    agent_data = [
        ...
        ['Hotel Search Agent', '2.1', 'Başarılı', 'Yakındaki oteller bulundu'],
    ...
    story.append(Paragraph(
        f"• {len(hotel_data) if hotel_data else 0} otel seçeneği analiz edildi",
        normal_style
    ))
    story.append(Paragraph(
    ...
    story.append(Paragraph("• Otel seçenekleri ile fiyat rekabeti artırılabilir", normal_style))
...
def simulate_agent_with_details(agent_name, agent_function, document, step_number, results=None, location=None):
    ...
    with st.expander(f"🤖 **Agent {step_number}: {agent_name}**", expanded=True):
        ...
        if agent_name == "Document Processor":
            ...
            """)
        elif agent_name == "Hotel Search Agent":
            st.info("🏨 **Girdi:** Fırsat konumu ve gereksinimler")
            st.code(f"""
            Görev: Yakındaki otelleri ara
            - Konum: {location or 'Washington DC'}
            - Yarıçap: 50 km
            - Kapasite: 100+ kişi
            - Konferans odaları
            - AV ekipmanı
            ...
            st.info("📊 **Girdi:** Tüm işlem sonuçları, metrikler ve otel verileri")
            st.code("""
        ...
        try:
            if agent_name == "Hotel Search Agent":
                # Otel arama simülasyonu
                hotels = search_nearby_hotels(location or "Washington DC")
                
                st.success(f"✅ **Sonuç:** {len(hotels)} otel bulundu")
                
                # Otel listesini göster
                for i, hotel in enumerate(hotels[:3], 1):
                    with st.expander(f"🏨 {hotel['name']}", expanded=False):
                        col1, col2 = st.columns(2)
                        with col1:
                            st.write(f"**Adres:** {hotel['address']}")
                            st.write(f"**Puan:** {hotel['rating']}/5.0")
                            st.write(f"**Fiyat:** {hotel['price_range']}")
                        with col2:
                            st.write(f"**Kapasite:** {hotel['capacity']} kişi")
                            st.write(f"**Mesafe:** {hotel['distance']}")
                            st.write(f"**Olanaklar:** {', '.join(hotel['amenities'][:2])}")
                
                if len(hotels) > 3:
                    st.info(f"... ve {len(hotels)-3} otel daha bulundu")
                
                return {"hotels": hotels, "location": location}
                ...
                st.success("✅ **Sonuç:** Üst yönetim PDF raporu oluşturuldu (Otel analizi dahil)")
                st.json({
                    "report_type": "Executive Summary + Hotel Analysis",
                    "pages": "6-8 sayfa",
                    "sections": [
                        "Executive Summary",
                        "Ana Metrikler",
                ...
                # PDF indirme butonu
                st.download_button(
                    label="📥 PDF Raporunu İndir (Otel Analizi Dahil)",
                    data=pdf_buffer.getvalue(),
                    file_name=f"ZgrBid_AutoGen_Hotel_Report_{datetime.now().strftime('%Y%m%d_%H%M')}.pdf",
...
def main():
    st.set_page_config(
        page_title="ZgrBid Live AutoGen Demo - Hotel Search",
        page_icon="🏨",
        layout="wide"
    )
    
    st.title("🏨 ZgrBid Live AutoGen Demo - Otel Arama Agent ile")
    st.markdown("---")
    
    # Sidebar
    st.sidebar.title("🎛️ Kontrol Paneli")
    
    if st.sidebar.button("🚀 Canlı AutoGen + Otel Arama Başlat", type="primary"):
        run_live_autogen_with_hotels()
...
def run_live_autogen_with_hotels():
    """Canlı AutoGen + Otel arama demo'sunu çalıştır"""
    
    st.header("🚀 Canlı AutoGen + Otel Arama İşlem Süreci")
    ...
    for opp_idx, opp in enumerate(opportunities, 1):
        ...
        # Agent 7: Hotel Search Agent
        hotel_result = simulate_agent_with_details(
            "Hotel Search Agent",
            None,  # Hotel arama fonksiyonu parametre olarak geçilmiyor
            document, 7,
            None,  # results gerekmiyor
            "Washington DC"  # Varsayılan konum
        )
    ...
    # Otel Analizi Özeti
    st.subheader("🏨 Otel Analizi Özeti")
    ...
    if all_hotels:
        ...
        st.write("**En Yüksek Puanlı Oteller:**")
    ...
    if pdf_result and pdf_result.get('pdf_generated'):
        ...
        with col1:
            ...
            st.info(f"📄 Sayfa Sayısı: 6-8 sayfa")
    ...
    st.balloons()
    st.success("🎉 Canlı AutoGen + Otel Arama işlemi başarıyla tamamlandı!")
