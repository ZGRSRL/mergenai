#!/usr/bin/env python3
"""
Streamlit Complete AutoGen Demo - Mail Agent ile
"""
...
def create_executive_pdf_report(results, total_metrics, hotel_data=None):
    ...
    try:
        ...
        try:
            # Linux'ta bulunan DejaVu fontunu kullan
    ...
    # Başlık
    story.append(Paragraph("ZgrSam AutoGen Sistemi", title_style))
    ...
    agent_data = [
        ...
        ['Email Sender Agent', '2.2', 'Başarılı', 'Rapor e-posta ile gönderildi']
    ]
    ...
    story.append(Paragraph(
        f"• Rapor otomatik olarak e-posta ile gönderildi",
        normal_style
    ))
    story.append(Paragraph(
    ...
    # Footer
    story.append(Paragraph("Bu rapor ZgrSam AutoGen sistemi tarafından otomatik oluşturulmuştur.", 
...
def send_email_with_pdf(recipients, subject, body, pdf_buffer, filename):
    ...
    if not sender_email or not sender_password:
        return False, "SMTP_EMAIL veya SMTP_PASSWORD .env dosyasında bulunamadı!"
    ...
    try:
        ...
        # PDF ekle
        pdf_buffer.seek(0)
        attach = MIMEApplication(pdf_buffer.read(), _subtype="pdf")
...
def simulate_agent_with_details(agent_name, agent_function, document, step_number, results=None, opportunity=None):
    ...
    with st.expander(f"🤖 **Agent {step_number}: {agent_name}**", expanded=True):
        ...
        if agent_name == "Document Processor":
            ...
            st.code("""
            Görev: Üst yönetim için PDF raporu oluştur
            - Executive Summary
            - Ana metrikler tablosu
            - Akıllı otel analizi
            - Fırsat detayları
            ...
            st.info("📧 **Girdi:** PDF raporu ve alıcı bilgileri")
            st.code("""
            Görev: Raporu e-posta ile gönder
            - Alıcı: info@creataglobal.com
            - Konu: ZgrSam AutoGen Raporu
            - Ek: PDF raporu
            - HTML ve Plain Text format
            """)
        ...
        try:
            if agent_name == "Smart Location Analyzer":
                ...
                st.download_button(
                    ...
                    file_name=f"ZgrSam_Smart_Location_Report_{datetime.now().strftime('%Y%m%d_%H%M')}.pdf",
                    mime="application/pdf"
                )
                
                return {"pdf_generated": True, "metrics": total_metrics, "hotels": hotel_data, "pdf_buffer": pdf_buffer}
            
            elif agent_name == "Email Sender Agent":
                # E-posta gönder
                pdf_buffer = results[0].get('pdf_buffer') if results and results[0].get('pdf_buffer') else None
                
                if not pdf_buffer:
                    st.error("❌ **Hata:** PDF raporu bulunamadı!")
                    return {"email_sent": False, "error": "PDF raporu bulunamadı"}
                
                recipients = ["info@creataglobal.com"]
                subject = f"ZgrSam AutoGen Raporu - {datetime.now().strftime('%d.%m.%Y %H:%M')}"
                body = f"""
Merhaba,

ZgrSam AutoGen sistemi analiz sonuçları ektedir.

📊 Özet:
- {len(results)} fırsat işlendi
- %{sum(r['compliance_matrix'].get('met_requirements', 0) for r in results) / sum(r['compliance_matrix'].get('total_requirements', 1) for r in results) * 100:.1f} compliance
- ${sum(r['pricing'].get('grand_total', 0) for r in results):,.0f} toplam değer
- {sum(len(r.get('hotels', [])) for r in results)} otel seçeneği analiz edildi
...
ZgrSam AutoGen Sistemi
                ...
                filename = f"ZgrSam_AutoGen_Report_{datetime.now().strftime('%Y%m%d_%H%M')}.pdf"
                
                success, message = send_email_with_pdf(recipients, subject, body, pdf_buffer, filename)
                
                if success:
                    st.success(f"✅ **Sonuç:** E-posta başarıyla gönderildi!")
                    st.json({
                ...
                return {"email_sent": success, "message": message, "recipients": recipients}
...
def main():
    st.set_page_config(
        page_title="ZgrSam Complete AutoGen Demo",
        page_icon="📧",
        layout="wide"
    )
    
    st.title("📧 ZgrSam Complete AutoGen Demo - Mail Agent ile")
    st.markdown("---")
    
    # Sidebar
    st.sidebar.title("🎛️ Kontrol Paneli")
    
    if st.sidebar.button("🚀 Complete AutoGen + Mail Başlat", type="primary"):
        run_complete_autogen_with_mail()
...
def run_complete_autogen_with_mail():
    """Complete AutoGen + Mail demo'sunu çalıştır"""
    
    st.header("🚀 Complete AutoGen + Mail İşlem Süreci")
    ...
    # PDF buffer'ı sonuçlara ekle
    if pdf_result and pdf_result.get('pdf_generated'):
        all_results[0]['pdf_buffer'] = pdf_result.get('pdf_buffer')
    
    # Agent 10: Email Sender Agent
    st.markdown("### 📧 **E-posta Gönderiliyor...**")
    email_result = simulate_agent_with_details(
        "Email Sender Agent",
        None,  # E-posta fonksiyonu parametre olarak geçilmiyor
        None,  # Document gerekmiyor
        10,
        all_results  # Tüm sonuçları geç
    )
    ...
    # E-posta Durumu
    if email_result and email_result.get('email_sent'):
        st.subheader("📧 E-posta Durumu")
        st.success("✅ Rapor başarıyla gönderildi!")
        st.info(f"📧 Alıcı: {', '.join(email_result.get('recipients', []))}")
        st.info(f"📅 Tarih: {datetime.now().strftime('%d.%m.%Y %H:%M:%S')}")
    else:
        st.subheader("📧 E-posta Durumu")
        st.error("❌ E-posta gönderilemedi!")
        if email_result:
            st.error(f"Hata: {email_result.get('message', 'Bilinmeyen hata')}")
    
    # PDF İndirme
    if pdf_result and pdf_result.get('pdf_generated'):
        st.subheader("📥 PDF İndirme")
        st.success("✅ PDF raporu hazır!")
        st.info("📄 Raporu indirmek için yukarıdaki 'PDF Raporunu İndir' butonunu kullanın.")
    
    conn.close()
    
    st.balloons()
    st.success("🎉 Complete AutoGen + Mail işlemi başarıyla tamamlandı!")
